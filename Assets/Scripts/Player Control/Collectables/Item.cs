...
public class Item : MonoBehaviour
{
    ...
    public virtual void Consume(ref PlayerData data) 
    {
        if (isInit) {
            ...
            if (SpeedIncrease > 0)
                data.Speed = Mathf.Max(Mathf.Min(data.Speed + SpeedIncrease * GlobalManager.GlobalInstance.ItemStrengthPercentage / 100 * GlobalManager.GlobalInstance.PotionSpeedPercentage / 100 + (SpeedIncrease > 0 ? 1 : 0) * (GlobalManager.GlobalInstance.ItemStrengthFlat + GlobalManager.GlobalInstance.PotionSpeedFlat), 40f),5f);
