class MapGenerator
{
    /* Note from Mathé: I wrote this helper struct Vector2Int because I couldn't include the corresponding
     * Unity struct into my local project for some reason. When part of the Unity project, we can just
     * comment this thing out and use the usual Unity-Vector2Int instead. Everything should (fingers crossed) be
     * compatible.
     *
    struct Vector2Int
    {
        ...
        public Vector2Int(int xx = 0, int yy = 0)
        {
            x = xx;
            y = yy;
        }
    ...
    int[, ] map; // For 0 <= x < mapWidth and 0 <= y < mapHeight we will have:
                  ...
                  // map[x,y] > 0     means tile at (x,y) is walkable ground; the value of map[x,y]
                  //                                   denotes the room number this tile is part of
                  // map[x,y] < 0    means tile at (x,y) is a door; the absolute value of map[x,y]
                  //                                   denotes the door number this tile belongs to
    ...
    Dictionary<int, Vector2Int> room_location; // keys are room numbers of already existing rooms
                                            ...
                                            // number r
                                            // So if room_location[r] == (x,y), then we know that map[x,y] == r
    ...
    private const int FREE = 9999997; // constants to denote free/full room space. Should be bigger than
    private const int FULL = 9999998; // roomCounter will ever be
    private const int BLOCKED = 9999999;
    ...
    public void printMap(int size, int x_topLeft, int y_topLeft)
    {
        ...
        for (int y = y_topLeft; y < y_topLeft + size; y++)
        {
            ...
            {
                string str = map[x, y] == 0 ? "#" : map[x, y].ToString();
    ...
    private void print_scratchpad(int[,] scratchpad)
        {
            ...
            for (int y = 0; y < maxRoomHeight; y++)
            {
                ...
                {
                    char character = scratchpad[x, y] == FREE ? ' ' : '#';
    ...
    private bool place_connected_door(int[,] scratchpad, Door door, Vector2Int doorPos)
    {
        ...
        // to.
        connected_doors.Add(door.doorNumber, new Tuple<int, int>(roomCounter, door.room(map)));
        return true;
    }
    ...
    private bool place_new_door(int[,] scratchpad, Vector2Int inwards, Vector2Int doorPos)
    {
        ...
        else // outwards is UP or DOWN
        {
            ...
            int block_y = doorPos.y + inwards.x;
            for (int block_x = leftDoorPos.x - 1; block_x <= rightDoorPos.x + 1; block_x++)
                if (scratchpad[block_x, block_y] == FREE)
                    scratchpad[block_x, block_y] = BLOCKED;
    ...
    private void draw_scratchpad_on_map(Rect scratchpadRect, int[,] scratchpad)
    {
        for (int x = 0; x < maxRoomWidth; x++)
        {
            for (int y = 0; y < maxRoomHeight; y++)
            {
                ...
                {
                    map[x + scratchpadRect.topLeft.x, y + scratchpadRect.topLeft.y] = roomCounter;
                }
